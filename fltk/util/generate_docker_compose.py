...
def generate_p23_freezoff_w9s3_fast():
    template_path = get_deploy_path('p23_freezoff_w9s3_fast')
    num_clients = 9
    cpu_per_client = 1
    num_cpus = 20
    world_size = num_clients + 1
...
def generate_check_w18_fast():
    template_path = get_deploy_path('p11_freezoff_fast')
    num_clients= 6
    world_size = num_clients + 1
    system_template: dict = load_system_template(template_path=template_path)
...
def run(name, num_clients = None, medium=False):
    exp_dict = {
        ...
        'p23_w9s3_fast': generate_p23_freezoff_w9s3_fast,
        ...
        'generate_check_w18_fast': generate_check_w18_fast
    }
    if num_clients:
